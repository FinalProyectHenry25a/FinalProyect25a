...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const { byRam, byRom, byBrand, byPrice, byNetwork, byProcessor, byOrder } =
      ...
      //precio minimo y maximo en un array; ejemplo [500, 1000]
      if (byPrice !== null) {
        forFilter = forFilter.filter( (el) => el.dataValues.price >= byPrice[0] && el.dataValues.price <= byPrice[1]  )
      }
